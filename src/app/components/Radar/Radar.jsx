...
export const Radar = ({ className }) => {
  ...
  return (
    <div
      className={twMerge(
        "relative flex h-20 w-26 items-center justify-center rounded-full  ",
        className
      )}
    >
      <div
      ...
      >
        ...
        <div className="relative z-40 h-[2px] w-full bg-gradient-to-r from-transparent  via-sky-600 to-transparent" />
      </div>
      {/* concentric circles */}
      {circles.map((circle, idx) => (
        <Circle
          style={{
            height: `${(idx + 1) * 5}rem`,
            width: `${(idx + 1) * 5}rem`,
            border: `1px solid rgba(71, 85, 225, ${1 - (idx + 1) * 0.1})`,
          }}
          key={`motion-${idx}`}
          idx={idx}
        />
